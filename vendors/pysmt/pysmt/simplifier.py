...
from pysmt.utils import ImmutableAnnotations, set_bit
...
class Simplifier(pysmt.walkers.DagWalker):
    ...
    def walk_ge(self, formula, args, **kwargs):
        assert len(args) == 2
        return self.walk_lt(None, [args[1], args[0]], **kwargs)

    def walk_gt(self, formula, args, **kwargs):
        assert len(args) == 2
        return self.walk_le(None, [args[1], args[0]], **kwargs)
    ...
    def walk_let(self, formula, args, **kwargs):
        ...
        for v, e in formula.let_defs():
            variables.append(v)
            expressions.append(self.memoization[e])

        # trivial case where the body of the let expression is a single variable
        if args[0] in variables:
            return expressions[variables.index(args[0])]

        return self.manager.Let(variables, expressions, args[0])
    ...
    def walk_annotation(self, formula, args, **kwargs):
        # TODO: also annotations defined as FNode can in principle be simplified
        return self.manager.Annotation(args[0], formula.get_annotations())
    ...
    def walk_bv_nand(self, formula, args, **kwargs):
        assert len(args) == 2
        return self.manager.BVNand(args[0], args[1])

    def walk_bv_nor(self, formula, args, **kwargs):
        assert len(args) == 2
        return self.manager.BVNor(args[0], args[1])
    ...
    def walk_bv_xnor(self, formula, args, **kwargs):
        assert len(args) == 2
        return self.manager.BVXnor(args[0], args[1])
    ...
    def walk_bv_ugt(self, formula, args, **kwargs):
        ...
        return self.manager.BVUGT(args[0], args[1])

    def walk_bv_uge(self, formula, args, **kwargs):
        # TODO
        return self.manager.BVUGE(args[0], args[1])
    ...
    def walk_bv_smod(self, formula, args, **kwargs):
        # TODO
        return self.manager.BVSMod(args[0], args[1])
    ...
    def walk_floor_div(self, formula, args, **kwargs):
        ...
        if sl.is_int_constant() and sr.is_int_constant() and not sr.is_zero():
            l = sl.constant_value()
            r = sr.constant_value()
            # smtlib2 semantics of integer division:
            # r > 0 : l / r == floor(float(l) / r)
        ...
        if sl.is_int_constant():
            if sl.is_zero():
                return sl

        if sr.is_int_constant():
            if sr.is_one():
                return sl
    ...
    def walk_mod(self, formula, args, **kwargs):
        ...
        if sl.is_int_constant() and sr.is_int_constant() and not sr.is_zero():
            ...
            # TODO: check smtlib semantics and evaluate
            # if r > 0:
            #     return self.manager.Int(math.floor(float(l) / r))
            # if r < 0:
            #     return self.manager.Int(math.ceil(float(l) / r))
    ...
    def walk_abs(self, formula, args, **kwargs):
        ...
        if arg.is_int_constant():
            v = arg.constant_value()
            if v >= 0:
                return self.manager.Int(v)
            else:
                return self.manager.Int(-v)

        return self.manager.Abs(arg)
