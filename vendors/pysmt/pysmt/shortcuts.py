...
# TODO: add regexes, rounding modes, and realtoint, abs, mod, floordiv, all others
...
def Annotation(fnode: FNode, annotations: ImmutableAnnotations):
    """Returns the formula in fnode augmented with annotations.

    :param fnode: the formula to be annotated
    :param annotations: the annotations
    :returns: Annotated fnode
    """
    return get_env().formula_manager.Annotation(fnode, annotations)
...
def write_smtlib(formula, fname, daggify:bool=True):
    ...
    with open(fname, "w") as fout:
        ...
        script.serialize(fout, daggify=daggify)
