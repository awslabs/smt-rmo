...
class Substituter(pysmt.walkers.IdentityDagWalker):
    ...
    def _push_with_children_to_stack(self, formula, **kwargs):
        ...
        # Deal with let
            ...
            substitutions = kwargs["substitutions"]
            new_subs = {}
            bounded_vars = [v for v,e in formula.let_defs()]
            for k,v in substitutions.items():
                # If at least one bound variable is in the cone of k,
                # we do not consider this substitution in the body of
                # the quantifier.
                if all(m not in bounded_vars
                       for m in k.get_free_variables()):
                    new_subs[k] = v

            # 2. we apply the substitution on the bounded expressions
            variables, expressions = [], []
            for v, e in formula.let_defs():
                sub = self.__class__(self.env)
                res_formula = sub.substitute(e, new_subs)
                variables.append(v)
                expressions.append(res_formula)
            ...
            res = self.mgr.Let(variables, expressions, res_formula)
...
class MGSubstituter(Substituter):
    ...
    @handles(set(op.ALL_TYPES) - op.QUANTIFIERS - {op.FUNCTION, op.LET})
    ...
    def walk_annotation(self, formula, args, **kwargs):
        ...
        if res is None:
            ...
            res = self.mgr.Annotation(args[0], new_annotations)
