...
from pysmt.typing import BOOL, REAL, INT, REGEX, BVType, STRING
...
FNodeContentBackend = collections.namedtuple("FNodeContentBackend", ["node_type", "args", "payload"])

class FNodeContent(FNodeContentBackend):
    # For some nodes, we may store additional information about their original representation
    # in the input smt file. This information is kept to allow for a consistent serialization
    # of the value but does not add to the semantics of the node itself. For example, the same
    # bitvector constant can be written with different notations in the input file.

    def __eq__(self, __value: object) -> bool:
        if isinstance(__value, FNodeContent):
            if self.node_type == BV_CONSTANT:
                return self.node_type == __value.node_type \
                        and self.args == __value.args \
                        and self.payload[0] == __value.payload[0] \
                        and self.payload[1] == __value.payload[1]
        
        return super().__eq__(__value)

    def __hash__(self) -> int:
        if self.node_type == BV_CONSTANT:
            return hash(FNodeContentBackend(
                node_type=BV_CONSTANT,
                args=tuple(),
                payload=(self.payload[0], self.payload[1], False, False)
            ))
...
class FNode(object):
    ...
    def is_le(self):
        """Test whether the node is the LE (less than or equal) relation."""
        return self.node_type() == LE

    def is_ge(self):
        """Test whether the node is the GE (greater than or equal) relation."""
        return self.node_type() == GE
    ...
    def is_gt(self):
        """Test whether the node is the GT (greater than) relation."""
        return self.node_type() == GT
    ...
    def is_bv_nand(self):
        """Test whether the node is the BVNand operator."""
        return self.node_type() == BV_NAND
    ...
    def is_bv_nor(self):
        """Test whether the node is the BVNOr operator."""
        return self.node_type() == BV_NOR
    ...
    def is_bv_xnor(self):
        """Test whether the node is the BVXnor operator."""
        return self.node_type() == BV_XNOR
    ...
    def is_bv_ugt(self):
        """Test whether the node is the BVUGT (unsigned greater than) relation."""
        return self.node_type() == BV_UGT

    def is_bv_ule(self):
        """Test whether the node is the BVULE (unsigned less or equal than) relation."""
        return self.node_type() == BV_ULE
    
    def is_bv_uge(self):
        """Test whether the node is the BVUGE (unsigned greater or equal than) relation."""
        return self.node_type() == BV_UGE
    ...
    def is_bv_smod(self):
        """Test whether the node is the BVSMod (signed modulus) operator."""
        return self.node_type() == BV_SMOD
    ...
    def is_re_op(self):
        """Test whether the node is a regex operator."""
        return self.node_type() in REGEX_OPERATORS

    @lru_cache
    def is_re_const(self):
        """Test whether the node is a regex constant."""
        if self.node_type() in {RE_ALL, RE_ALLCHAR, RE_NONE}:
            return True
        if self.node_type() == RE_RANGE:
            return (
                self.args()[0].is_string_constant()
                and self.args()[1].is_string_constant()
            )
        return False

    def is_re_loop(self):
        """Test whether the node is a regex loop."""
        return self.node_type() == RE_LOOP

    def re_loop_bounds(self):
        """Returns the bounds n1 and n2 of a regex loop expression with (_ re.loop n1 n2)"""
        assert self.is_re_loop()
        return self._content.payload[0], self._content.payload[1]

    def is_re_power(self):
        """Test whether the node is a regex power."""
        return self.node_type() == RE_POWER

    def is_int_to_bv(self):
        return self.node_type() == INT_TO_BV

    def is_bv_to_int(self):
        return self.node_type() == BV_TO_INT

    def re_power_exponent(self):
        assert self.is_re_power()
        """Returns the exponent n of a regex power expression with (_ re.^ n)"""
    ...
    def constant_value(self):
        ...
        if self.node_type() == BV_CONSTANT or self.node_type() == STR_CONSTANT:
            return self._content.payload[0]
    ...
    def constant_type(self):
        ...
        if self.node_type() == INT_CONSTANT:
            ...
            return STRING
        elif self.node_type() in REGEX_CONSTANTS:
            return REGEX
        else:
    ...
    def bv_underscore(self):
        assert self.is_bv_constant()
        return self._content.payload[2]

    def bv_hex(self):
        assert self.is_bv_constant()
        return self._content.payload[3]
    
    def str_unary_hex(self):
        assert self.is_string_constant()
        return self._content.payload[1]
    ...
    def let_defs(self):
        """Return the list of definitions of a `let`."""
        assert self.is_let()
        return self._content.payload
    ...
    def constructor(self):
        assert self.is_is()
        # case 1: constructor with params (represented as partial function)
        if isinstance(self._content.payload, functools.partial):
            return self._content.payload.args[0]
        # case 2: nullary constructor (represented as symbol)
        return self._content.payload

    def is_annotation(self):
        return self.node_type() == ANNOTATION

    def get_annotations(self):
        assert self.is_annotation()
        return self._content.payload[0]
    ...
    def __mod__(self, right):
        return self._apply_infix(right, _mgr().Mod, bv_function=_mgr().BVURem)
