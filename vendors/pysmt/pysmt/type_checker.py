...
from pysmt.typing import BOOL, REAL, INT, ROUNDING_MODE, BVType, ArrayType, STRING, REGEX
...
class SimpleTypeChecker(walkers.DagWalker):
    ...
    @walkers.handles(op.FLOOR_DIV, op.MOD)
    ...
    @walkers.handles(op.BV_ADD, op.BV_SUB, op.BV_NOT, op.BV_AND, op.BV_OR, op.BV_NAND, op.BV_NOR)
    @walkers.handles(op.BV_XOR, op.BV_XNOR, op.BV_NEG, op.BV_MUL)
    @walkers.handles(op.BV_UDIV, op.BV_UREM, op.BV_LSHL, op.BV_LSHR)
    @walkers.handles(op.BV_SDIV, op.BV_SREM, op.BV_SMOD, op.BV_ASHR)
    ...
    def walk_str_to_str_re(self, formula, args, **kwargs):
        #pylint: disable=unused-argument
        assert formula is not None
        if len(args) == 2:
            if args[0].is_regex_type() and args[1].is_string_type():
                return STRING
        return None

    @walkers.handles(op.RE_RANGE)
    def walk_re_range(self, formula, args, **kwargs):
        #pylint: disable=unused-argument
        assert formula is not None
        if len(args) == 2:
            if args[0].is_string_type() and args[1].is_string_type():
                return REGEX
        return None

    @walkers.handles(op.RE_POWER)
    def walk_re_power(self, formula, args, **kwargs):
        #pylint: disable=unused-argument
        assert formula is not None and formula.is_re_power()
        if len(args) == 1:
            if args[0].is_regex_type():
                return REGEX
        return None

    @walkers.handles(op.RE_LOOP)
    def walk_re_loop(self, formula, args, **kwargs):
        #pylint: disable=unused-argument
        assert formula is not None and formula.is_re_loop()
        if len(args) == 1:
            if args[0].is_regex_type():
                return REGEX
        return None

    @walkers.handles(op.RE_CONCAT, op.RE_INTER, op.RE_UNION, op.RE_STAR,
                     op.RE_COMP, op.RE_DIFF, op.RE_PLUS, op.RE_OPT)
    ...
    @walkers.handles(op.STR_IN_RE)
    def walk_str_in_re(self, formula, args, **kwargs):
        #pylint: disable=unused-argument
        assert formula is not None
        if len(args) == 2:
            if args[0].is_string_type() and args[1].is_regex_type():
                return BOOL
        return None

    @walkers.handles(op.RE_ALL, op.RE_ALLCHAR, op.RE_NONE)
    def walk_re_constant(self, formula, args, **kwargs):
        assert formula is not None
        return REGEX
    ...
    @walkers.handles(op.BV_ULT, op.BV_UGT, op.BV_ULE, op.BV_UGE, op.BV_SLT, op.BV_SLE)
    ...
    def walk_bv_concat(self, formula, args, **kwargs):
        ...
        if len(args) == 1:
            return BVType(args[0].width)
        try:
            concat_width = sum(a.width for a in args)
            target_width = formula.bv_width()
        except AttributeError:
            return None
        if not concat_width == target_width:
            return None
    ...
    def walk_int_to_bv(self, formula, args, **kwagrs):
        if len(args) != 1 or not args[0].is_int_type():
            return None
    ...
    @walkers.handles(op.LE, op.LT, op.GE, op.GT)
    ...
    @walkers.handles(op.RNE, op.RNA, op.RTP, op.RTN, op.RTZ)
    def walk_rounding_mode(self, formula, args, **kwargs):
    ...
    def walk_is(self, formula, args, **kwargs):
        # constructor in is is either a partial function or a symbol (nullary)
        if not isinstance(formula._content.payload, functools.partial):
            if not formula._content.payload.is_symbol():
                return None
        # TODO check that it is indeed a constructor of the ADT
        return BOOL
        
    @walkers.handles(op.LET)
