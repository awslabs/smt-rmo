...
class Theory(object):
    ...
    def __init__(self,
        ...
        self.regexes = regexes or False
        assert not arrays_const or arrays, "Cannot set arrays_const w/o arrays"
        assert not regexes or strings, "Cannot set regexes w/o strings"
        return
    ...
    def set_regexes(self, value=True):
        res = self.copy()
        res.regexes = value
        res.strings = res.strings or value
        return res
    ...
    def combine(self, other):
        ...
        return Theory(
            ...
            regexes=self.regexes or other.regexes)
...
# TODO: there official list of smtlib 2.6 logics is still incomplete: https://smtlib.cs.uiowa.edu/logics.shtml
# addind them one when we encounter them until the official list is released

QF_BVRE = Logic(name="QF_BVRE",
                description=\
                    """Quantifier-free formulas over the theories of fixed-size bitvectors and regular expressions""",
                bit_vectors=True,
                quantifier_free=True,
                strings=True,
                regexes=True)
...
ALL = Logic(name="ALL",
            description=\
                """Most generic logic, accepted by some solvers in practice""",
            arrays = True,
            arrays_const = True,
            bit_vectors = True,
            floating_point = True,
...
LOGICS = SMTLIB2_LOGICS | frozenset([QF_BOOL, BOOL, QF_AUFBVLIRA, QF_NIRA]) | frozenset([QF_BVRE, ALL])
...
def get_logic_by_name(name):
    ...
    # Fallback: create an arbitrary new logic instead of raising UndefinedLogicError(name)
    return name
...
def get_logic(quantifier_free=False,
    ...
    for logic in LOGICS:
        if (logic.quantifier_free == quantifier_free and
            ...
            logic.theory.strings == strings and
            logic.theory.regexes == regexes):
            return logic
