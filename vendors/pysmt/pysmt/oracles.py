...
class TheoryOracle(walkers.DagWalker):
    # TODO: add regexes
    # TODO: add floordiv, mod, and abs, which will make the theory support non-linear integer operations
    # TODO: add datatypes
...
class FreeVarsOracle(walkers.DagWalker):
    ...
    def walk_let(self, formula, args, **kwargs):
        ...
        # return args[0].difference([v for v, e in formula.let_defs()])
        res = set()
        res.update(args[0])
        for v, e in formula.let_defs():
            res.update(self.walk(e, **kwargs))
            res = res.difference([v])
        return res

    @walkers.handles(op.ANNOTATION)
        ...
        for k in annotations:
            ...
            for v in values:
                if isinstance(v, tuple):
                    ...
                    for sexpr in v:
                        res.update(self.walk(sexpr, **kwargs))
                elif isinstance(v, FNode):
                    res.update(self.walk(v, **kwargs))
...
class TypesOracle(walkers.DagWalker):
    ...
    def walk_function(self, formula, args, **kwargs):
        ...
        res = frozenset([ftype.return_type] + list(ftype.param_types))
        if args is not None:
            for arg in args:
                res = res | arg
        return res
