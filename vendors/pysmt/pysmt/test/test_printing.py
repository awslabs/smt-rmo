...
from pysmt.shortcuts import TRUE, Annotation, Or, And, Not, Plus, Iff, Implies
...
class TestPrinting(TestCase):
    ...
    def test_quantifiers(self):
        ...
        self.assertEqual(fa.to_smtlib(daggify=True),
                         "(let ((.def_0 (forall ((x Bool)) (let ((.def_1 (not x))) (let ((.def_2 (and x .def_1))) .def_2))))) .def_0)")
        self.assertEqual(fe.to_smtlib(daggify=True),
                         "(let ((.def_0 (exists ((x Bool)) (let ((.def_1 (not x))) (let ((.def_2 (and x .def_1))) .def_2))))) .def_0)")
    ...
    def test_constant(self):
        ...
        self.assertEqual(r1.to_smtlib(daggify=True), "5.5")
        self.assertEqual(r2.to_smtlib(daggify=True), "5.0")
        self.assertEqual(r3.to_smtlib(daggify=True), "(- 5.5)")
        ...
        self.assertEqual(r1.to_smtlib(daggify=False), "5.5")
        self.assertEqual(r2.to_smtlib(daggify=False), "5.0")
        self.assertEqual(r3.to_smtlib(daggify=False), "(- 5.5)")
    ...
    def test_annotations(self):
        ...
        annotations_for_x = ImmutableAnnotations()
        annotations_for_x = annotations_for_x.add('next', x_next.symbol_name())
        annotations_for_x = annotations_for_x.add('init', TRUE())

        x_annotated = Annotation(x, annotations_for_x)

        f = Iff(x_annotated, Not(x_next))
        annotations_for_f = ImmutableAnnotations(('trans', ), ((TRUE(), ), ) )

        f_annotated = Annotation(f, annotations_for_f)
        ...
        dag_printer.printer(f_annotated)
        tree_printer.printer(f_annotated)

        self.assertEqual(tree_buf.getvalue(),
                         "(! (= (! x :next x.next :init true) (not x.next)) :trans true)")
        self.assertEqual(dag_buf.getvalue(),
                         "(let ((.def_0 (not x.next))) (let ((.def_1 (! x :next x.next :init true)) (let ((.def_2 (= .def_1 .def_0))) (let ((.def_3 (! .def_2 :trans true)) .def_3))))")
